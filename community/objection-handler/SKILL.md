...
description: Sales objection handling framework using MEDDIC/BANT - converts 1h prep → 5min per call. Use when user wants to handle "client says too expensive", "client hesitates", "delays signing", "client wants to downgrade", "competitor named", "need to check with boss", "not a priority right now", "call us next quarter", "price too high", "需要降价", "客户犹豫", "客户要降级", "客户拖延", "客户质疑 ROI", "提案被驳回", "call us back later", "deal rescue", "proposal defense", "needs to consult", or any sales objection during deal cycle. Frequency: every sales interaction.
version: 2.0.0
tags: [sales, objection-handling, MEDDIC, BANT, deal-acceleration, ROI-9.5, community, arena-winner]
---
...
| # | 场景 | 触发词 |
|---|---|---|
| 1 | **价格异议** | 太贵 / 价格高 / 竞品便宜 / 客户要降价 / price too high |
| 2 | **决策权异议** | 客户说要跟老板商量 / need to check with boss / 不是我能决定的 |
| 3 | **需求异议** | 客户说不是优先级 / not a priority right now / 不急 |
| 4 | **信任异议** | 客户质疑效果 / 没有证据 / 用过的客户怎么评价 |
| 5 | **时机异议** | 客户拖延 / call us next quarter / 现在不合适 / 下个季度再聊 |
| 6 | **竞品异议** | 已经在用别家 / 评估中 / 跟 X 对比一下 |

### 5 类常见痛点

- 5 分钟内想不出怎么回应 — **话到嘴边变废话**
- 越辩越被动 — **给完方案对方提新疑点**
- 价格被压得过低 — **底线没守住,成交了反而亏**
- 客户说「我再想想」就再也联系不上 — **没设 next step**
- 团队每个人应对方法不一致 — **没有统一框架**

### 6 类期望效果

- 3-tier 响应(Acknowledge / Reframe / Advance)标准化输出
- MEDDIC 6 元素对齐(清晰度 ≥ 70%)
- 1 个具体 next step(会议/试用/参考客户)
- 应对时间 ≤ 5 分钟(从 objection 到方案)
- 不直接降价 — 用价值重构换价格
- 每次响应可复用为团队 playbook
...
| 类型 | 触发 | 动作 |
|---|---|---|
| **签约/法律阶段** | 客户问合同条款/合规问题 | 不替对方谈合同,转律师 + 继续其他 objection |
| **已流失客户挽回** | 客户说「我已经选了别家」 | 不强挽,转流失复盘流程(见 churn-risk-playbook) |
| **报价/折扣超过底线** | 客户要超过 30% 折扣 | 不擅自承诺,转销售经理决定 |
| **情绪化攻击** | 客户带情绪发泄 | 先 Acknowledge(共情),再判断真假 objection |
...
| 信息 | 必要性 | 缺失时默认 |
|---|---|---|
| **具体 objection 原话** | 必须 | 不开始,问 |
| **Deal stage**(discovery / demo / proposal / close) | 强烈推荐 | 假设 demo 阶段 |
| **MEDDIC 完整度** | 推荐 | 假设 50%,后续提醒补 |
| **竞争对手信息**(若有) | 推荐 | 不假设 |
| **本次目标**(close / 维持关系 / 拿到 next meeting) | 强烈推荐 | 假设拿到 next meeting |
...
| 反馈 | skill 动作 |
|---|---|
| 「客户没回应」 | 检查 Advance 步骤是否设了 next step |
| 「价格被压太多」 | 询问具体折扣% + 触发底线保护 |
| 「对方质疑 ROI」 | 触发 MEDDIC-Metrics 深化,补量化案例 |
| 「我用了但效果不好」 | 检查 MEDDIC 完整度,多半 < 50% 才会输 |

### skill → 主公主动迭代

| 周期 | 内容 |
|---|---|
| 每次用完 | 「deal 后续反馈?」(close 了 / 没了 / 续约) |
| 3 次使用后 | 「3 次下来,你最常碰的 objection 是 X,要不要存为 quick-pick?」 |
| 季度 | 「本季度 objection 类型分布:价格 30% / 时机 25% / ... 团队有哪些高频应对 playbook 需要更新?」 |

---

## 🛠 自检 Checklist

- [ ] Objection 类型判断对了?(price / authority / need / trust / timing / competition)
- [ ] MEDDIC 元素覆盖?
- [ ] Acknowledge 不打折扣、Reframe 不贬低客户、Advance 有 next step?
- [ ] 没直接降价?(即使客户施压)
- [ ] 没承诺超过底线?
- [ ] 应对话术 < 5 分钟能讲完?
- [ ] 这次输出可加入团队 playbook?

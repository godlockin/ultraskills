...
export async function writeSkill(id, content, cwd, opts = {}) {
  ...
  await assertSafeWriteTarget(skillsRoot, targetDir);
  await mkdir(targetDir, { recursive: true });
  await assertSafeWriteTarget(skillsRoot, targetFile);
...
export async function writeManifest(cwd, skill) {
  const skillsRoot = resolveWithin(cwd, '.claude', 'skills');
  const manifestPath = resolveWithin(skillsRoot, '.ultraskills-manifest.json');
  await mkdir(skillsRoot, { recursive: true });
  await assertSafeWriteTarget(skillsRoot, manifestPath);

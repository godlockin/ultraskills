...
export async function readManifest(cwd) {
  const manifestPath = resolveWithin(cwd, '.claude', 'skills', MANIFEST_NAME);
  try {
    const raw = await readFile(manifestPath, 'utf8');
    /** @type {Manifest} */
    const m = JSON.parse(raw);
    const ids = (m.skills ?? []).map(e => e.id).filter(Boolean).map(assertSafeSkillId);
  ...
  const skillsDir = resolveWithin(cwd, '.claude', 'skills');
    try {
      ...
      const dirs = entries.filter(e => e.isDirectory()).map(e => e.name).filter(id => {
        try { assertSafeSkillId(id); return true; } catch { return false; }
      });
      const ids = [];
...
export async function update(index, cwd, opts = {}) {
  ...
  for (const id of ids) {
    ...
    try {
      ...
      const destDir = resolveWithin(skillsDir, safeId);
      await mkdir(destDir, { recursive: true });
      await writeFile(resolveWithin(destDir, 'SKILL.md'), content, 'utf8');
      console.log(`  ✓ ${id}`);
      updated++;

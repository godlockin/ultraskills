...
**UltraSkills** is a 581-skill library for Codex. Modular, arena-ranked AI prompt patterns following the S.C.A.L.E. model. Primarily a documentation/prompt library.
...
│   ├── list.js                      # List skills
│   ├── install.js / uninstall.js    # Install/remove skills to ~/.Codex/skills
...
│   ├── deploy_skills.py             # 部署: scan + deploy 到 ~/.Codex/skills
...
│   ├── ultraskills-hub/             # ⭐ Hub 搜索 skill (Codex 入口)
│   │   └── scripts/search.py
...
| `setup.sh` | 安装 skills 到 ~/.Codex/skills/ |
...
ultraskills 仓库有 826+ skills，但只有一部分部署到 `~/.Codex/skills/`。
